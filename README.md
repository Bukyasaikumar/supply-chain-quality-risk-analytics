...
- **Python (pandas)** — automated exception detection, extending the SQL analysis
- **HTML/Chart.js** — web-based dashboard (no licence required, see `index.html`)
...
| `index.html` | Interactive web dashboard with a working product-type filter — open in any browser |
| `supply_chain_raw.csv` | Cleaned dataset used for analysis (100 rows, 24 columns, no nulls/duplicates) |
| `query_results/` | Screenshot of each query's output from MySQL Workbench |
| `exception_detection.py` | Python exception-detection script — extends the SQL analysis with automated outlier detection and configurable risk flagging |

## Python Exception Detection (extension)

`exception_detection.py` extends the SQL analysis above into a reusable, re-runnable exception-testing script — same underlying dataset and same supplier/product risk findings, but structured as a pipeline instead of a set of one-off queries:

- Data quality checks (nulls, duplicate rows, duplicate SKUs, invalid values)
- Statistical outlier detection on defect rates, calculated per product type rather than against a single dataset-wide threshold
- Threshold-based exception flagging for high-risk supplier/product combinations, with configurable thresholds (defect rate, revenue at risk)
- Console + CSV exception report, so results can be reviewed without re-running the script

Run it with:

```
python exception_detection.py
```
